...
;;; A counter shared between processes needs a real compare-and-swap. The children wait on a
;;; start flag in word 1 so that they are guaranteed to overlap rather than run one after another.
(test shmem-cas-cross-process
      (let ((name (%shm-name))
            (size 4096)
            (nproc 4)
            (ninc 20000))
        (unwind-protect
             (let* ((m (clasp-posix:open-shared-memory name size :create t))
                    (counter (clasp-ffi:%foreign-data-address (clasp-posix:mapping-pointer m)))
                    (flag (+ counter 4)))
               (clasp-ffi:%mem-set-uint32 counter 0)
               (clasp-ffi:%mem-set-uint32 flag 0)
               (finish-output)
               (finish-output *error-output*)
               (let ((kids '()))
                 (dotimes (i nproc)
                   (multiple-value-bind (stream pid) (clasp-posix:fork nil)
                     ...
                     (cond
                       ((zerop pid)
                        (loop until (plusp (clasp-ffi:%mem-ref-uint32 flag)))
                        (dotimes (j ninc)
                          (loop for old = (clasp-ffi:%mem-ref-uint32 counter)
                                until (eql old (clasp-ffi:%cas-mem-uint32 counter old (1+ old)))))
                        (core:cexit 0))
                       (t (push pid kids)))))
                 (clasp-ffi:%mem-set-uint32 flag 1)
                 (dolist (pid kids)
                   (declare (ignore pid))
                   (clasp-posix:wait)))
               (prog1 (list (clasp-ffi:%mem-ref-uint32 counter) (* nproc ninc))
                 (clasp-posix:close-shared-memory m)))
          (ignore-errors (clasp-posix:shm-unlink name))))
      ((80000 80000)))
