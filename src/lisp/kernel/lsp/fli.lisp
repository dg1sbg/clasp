...
;;;
;;; fli-specs.lisp, generated by the build, communicates the information in
;;; the C++ runtime to Lisp by defining the *foreign-type-specs* variable.
;;;
...
;;; *foreign-type-specs* is defined by the build only at compilation time,
;;; and is an alist of Lisp symbols to indices in the table.
...
(macrolet ((generate-methods ()
             `(progn
                ,@(loop for (type . index) in *foreign-type-specs*
                        for specform = `(load-time-value
                                         (elt *foreign-type-spec-table* ,index)
                                         t)
                        collect
                        `(defmethod %lisp-type->type-spec ((type (eql ',type)))
                           ,specform)
                        collect
                        ...
                        unless (eq type :void)
                          collect
                        `(defmethod %mem-ref (ptr (type (eql ',type)) &optional (offset 0))
                           (,(intern (concatenate 'string "%MEM-REF-" (string type))
                                     "CLASP-FFI")
