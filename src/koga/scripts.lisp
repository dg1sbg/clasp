...
(let ((s (open (core:argv 3) :if-exists :supersede :if-does-not-exist :create :direction :output)))
  (print *features* s)
  (close s))
(let ((o (open (core:argv 4) :if-exists :supersede :if-does-not-exist :create :direction :output)))
  ...
  (write-line \")\" o)
  (close o))
(let ((s (open (core:argv 5) :if-exists :supersede :if-does-not-exist :create :direction :output)))
  (print `(progn
         ,@(mapcar #'(lambda (cn)
       ...
       s)
  (close s))
(let ((s (open (core:argv 6) :if-exists :supersede :if-does-not-exist :create :direction :output)))
  (print `(eval-when (:compile-toplevel)
       ...
       s)
  (close s))
(let ((s (open (core:argv 7) :if-exists :supersede :if-does-not-exist :create :direction :output)))
  (print `(eval-when (:compile-toplevel)
          ,@(let ((ss '()))
              (flet ((frob (k v)
                       (if (constantp v)
        ...
        s)
  (close s))
(let ((s (open (core:argv 8) :if-exists :supersede :if-does-not-exist :create :direction :output)))
  (print `(in-package #:cmp) s)
  (print `(defvar +cxx-data-structures-info+ ',(llvm-sys:cxx-data-structures-info)) s)
  (close s))
(let ((s (open (core:argv 9) :if-exists :supersede :if-does-not-exist :create :direction :output)))
...
(let ((s (open (core:argv 10) :if-exists :supersede :if-does-not-exist :create :direction :output)))
...
(defmethod print-prologue (configuration (name (eql :compile-bytecode-image)) output-stream)
  (declare (ignore configuration))
  (print-asdf-stub output-stream t :cross-clasp :split-sequence)
  (format output-stream "
(destructuring-bind (character-names features &rest rest)
    (uiop:command-line-arguments)
  (destructuring-bind (inputs outputs sources)
      (split-sequence:split-sequence \"--\" rest :test #'equal)
    (cross-clasp:initialize character-names features)
    (cross-clasp:build inputs outputs sources
                       (mapcar (lambda (output)
                                 (merge-pathnames (make-pathname :type \"cfasl\") output))
                               outputs))))"))
...
(defmethod print-prologue (configuration (name (eql :compile-native-image)) output-stream)
  ...
  (print-asdf-stub output-stream t :cross-clasp :split-sequence)
  (format output-stream "
(let ((number-of-jobs (if (ext:getenv \"CLASP_BUILD_JOBS\")
  ...
  (destructuring-bind (character-names features &rest rest)
    ...
    (destructuring-bind (inputs outputs sources)
        (split-sequence:split-sequence \"--\" rest :test #'equal)
      (cross-clasp:initialize character-names features)
      (cross-clasp:build-native inputs outputs sources
                                (mapcar (lambda (output)
                                          (merge-pathnames (make-pathname :type \"cfasl\") output))
                                        outputs)
